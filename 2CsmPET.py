...
def default_thresholds():
    """	
    Default values. sigma_r/g = standard deviation for gaussian kernel of 
    gaussian_filter1d for red and green channel, set 5 by default. threshold_r/g = initial 
    threshold used for calculation of data specific threshold, set 0.01 by default. n = scales
    to multiply to, used by mz_fwt. Set to 3 by default. If n is odd directionality of the step
...
def change_thresholds(sigma_r, sigma_g, threshold_r, threshold_g, n, delete_red, delete_green):
    ...
    In addition the option to delete events manually is provided.
...
def delete_event_red(steps, delete_red):
    """	
    Performs deletion of events if the option 
...
def step_sizes_own(data, indices):
    """ 
    step_sizes_own determines the directionality of each step (index) by looking up
...
def calculate_threshold(raw_data, sigma, threshold):
    """ 
    calculates data-specific threshold.
    a default threshold of 0.01 is applied. Iterates over traces. Several normalization and resizing 
    steps of the data and substraction of mean intensity is followed by smoothing with 
    gaussian_filter1d. mz_fwt from step_detect.py calculates the multiscale product and find_steps of
    step_detect.py find steps above the initial threshold, that gets subsequently ordered in 
    positive and negative ones by step_size_own() function. Relative magnitudes of all steps 
    provided by find_steps are collected. The average of all relative magnitudes ("peak-heights")
    is returned as calculated threshold and subsequently serves as threshold for the actual
    analysis of data set. The average of relative magnitudes was empirically found to be suited 
    as data specific threshold.
    ...
    sigma = scalar
            Standard deviation for gaussian kernel. (for furhter 
...
def check_for_bleaching(red, green, bleaching_index=341):
    ...
    frame counts as a bleaching event if the signal remains below the average value from
    frame 5 to the current frame for longer than the number of frames specified by 
    bleaching_index. Set to 341 by default (--> 340 frames).
...
def simultancy(red, green, window):
    ...
    them in simultaneous and not simultaneous events. Events are sorted for absolute value
    ...
    window = int
                ...
                into simultaneous and not simultaneous ones. Events count as simultaneous
                if red and green event happen within the time window. Starting from 
...
def calculate_events(red, green):
    ...
    Returns several lists containing ON or OFF steps in red channel (on_peaks_red, off_peaks_red), 
    ON or OFF steps in green channel (on_peaks_green, off_peaks_green), dwell times of ON or OFF
    states of red channel (results_on1, results_off1) and green channel (results_on2, results_off2),
    unfiltered steps (input data) of red (times) and green channel (times_g) and updated steps in red (peaks) and
    green channel (peaks_g).
    """
    times = red
    times_g = green
...
Files supported: .txt files with spacebar as delimiter.
...
and confirmation of the found steps/dwell times. Finally the script creates a .csv file
that contains the ON and OFF dwell times for red and green channel as well as the absolute
number of transitions/steps that occured simultaneously and not simultaneously.
"""
